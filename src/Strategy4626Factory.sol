...
contract Strategy4626Factory {
    ...
    error AlreadyDeployed(address strategy);
    ...
    function newStrategy4626(address _vault) external returns (address) {
        ...
        string memory name = string(abi.encodePacked("stETH ", ERC20(_vault).symbol(), " Accumulator"));
        IStrategy4626Interface newStrategy = IStrategy4626Interface(address(new Strategy4626(ASSET, name, _vault)));
    ...
    function isDeployedStrategy(address _strategy) external view returns (bool) {
        address vault = address(IStrategy4626Interface(_strategy).vault());
        return deployments[vault] == _strategy;
    }
}
